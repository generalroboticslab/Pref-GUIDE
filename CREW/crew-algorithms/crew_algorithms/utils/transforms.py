...
class RandomShift(ObservationTransform):
    ...
    def __init__(
        ...
        self.trans = RandomAffine(degrees=(0, 0), translate=(a, b), scale=(1.0, 1.0), interpolation=InterpolationMode.BILINEAR)
...
class RandRotate(ObservationTransform):
    ...
    def __init__(
        ...
        self.trans = RandomRotation(degrees=(-d, d), interpolation=InterpolationMode.BILINEAR)
...
class RandRotateChoice(ObservationTransform):
    ...
    def _apply_transform(self, observation: torch.Tensor) -> torch.Tensor:
        observation = transforms.functional.rotate(observation, random.choice(self.ds), interpolation=InterpolationMode.BILINEAR)
        return observation
...
class AddCoordinates(ObservationTransform):
    ...
    def _apply_transform(self, observation: torch.Tensor) -> torch.Tensor:
        ...
        batch_grid = grid.unsqueeze(0).repeat(observation.shape[0], 1, 1, 1).to(observation.device)
