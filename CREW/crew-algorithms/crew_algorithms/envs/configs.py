...
class BowlingConfig(EnvironmentConfig):
    ...
    action_dims: int = 3
    unity_server_build_path_linux: Path = '../crew-dojo/Builds/Bowling-StandaloneLinux64-Server/Unity.x86_64'
    unity_server_build_path_osx: Path = '../crew-dojo/Builds/Bowling-StandaloneOSX-Server/Unity'
    log_folder_path: Path = '../UnityLogs'
...
class FindTreasureConfig(EnvironmentConfig):
    ...
    action_space: str = "continuous"
    action_dims: int = 2
    unity_server_build_path_linux: Path = '../crew-dojo/Builds/FindTreasure-StandaloneLinux64-Server/Unity.x86_64'
    unity_server_build_path_osx: Path = '../crew-dojo/Builds/FindTreasure-StandaloneOSX-Server/Unity'
    log_folder_path: Path = '../UnityLogs'
...
class HideAndSeek1V1Config(EnvironmentConfig):
    ...
    action_space: str = "continuous"
    action_dims: int = 2
    unity_server_build_path_linux: Path = '../crew-dojo/Builds/HideAndSeek_Single-StandaloneLinux64-Server/Unity.x86_64'
    unity_server_build_path_osx: Path = '../crew-dojo/Builds/HideAndSeek_Single-StandaloneOSX-Server/Unity'
    log_folder_path: Path = '../UnityLogs'
    human_delay_steps: int = 2
    no_graphics: bool = False
    time_scale: float = 1.0
...
class HideAndSeekConfig(EnvironmentConfig):
    ...
    num_hiders: int = 2
    num_seekers: int = 2
    unity_server_build_path_linux: Path = '../crew-dojo/Builds/HideAndSeek-StandaloneLinux64-Server/Unity.x86_64'
    unity_server_build_path_osx: Path = '../crew-dojo/Builds/HideAndSeek-StandaloneOSX-Server/Unity'
    log_folder_path: Path = '../UnityLogs'
    @property
...
@define(auto_attribs=True)
class WallJumpConfig(EnvironmentConfig):
    name: str = "wall_jump"
    num_agents: int = 1
    num_channels: int = 3
    num_stacks: int = 3
...
def register_env_configs() -> None:
    ...
    cs.store(group="envs", name="base_wall_jump", node=WallJumpConfig)
    cs.store(group="envs", name="base_hide_and_seek", node=HideAndSeekConfig)
    cs.store(group="envs", name="base_tetris", node=TetrisConfig)
