...
from crew_algorithms.pytorch_grad_cam.pytorch_grad_cam import GradCAM, HiResCAM, ScoreCAM, GradCAMPlusPlus, AblationCAM, XGradCAM, EigenCAM, FullGrad
from crew_algorithms.pytorch_grad_cam.pytorch_grad_cam.utils.model_targets import RawScoresOutputTarget
from crew_algorithms.pytorch_grad_cam.pytorch_grad_cam.utils.image import show_cam_on_image
...
def eval(cfg: Config):
    ...
    with open(path + f"explore_results_g.json", "w") as f:
        print("\nSaving explore results!!!!\n")
        json.dump(explore, f)
