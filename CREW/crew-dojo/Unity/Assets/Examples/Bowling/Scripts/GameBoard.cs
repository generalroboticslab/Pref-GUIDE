...
namespace Examples.Bowling
{
    public class GameBoard : MonoBehaviour
    {
        ...
        public void ExecuteActionVector(Vector3 action)
        {
            ...
            {
                var pos = _ball.position;
                pos.y = Math.Clamp(move_to, -2.5f, 2.5f); // May need to animate a smooth transition later
                _ball.position = pos;
                _shootDir = new Vector2(2.5f, 0.0f);
                _stage = GameStage.Shoot;
                steer_counter = 0;
        ...
        private void NextState()
        {
            if (!IsClient)
            {
                if (_stage == GameStage.Shoot)
                {
                    ...
                    {
                        if (steer_counter > steer_time) // May need to clip steer time
                        {
                ...
                else if (_stage == GameStage.Collect)
                {
                    ...
                    // {
                        ...
                        // Debug.Log($"{clearedPins},_{_score._prevClearedPins},_{immediate_score}");
