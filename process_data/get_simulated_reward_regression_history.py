...
from models import CNN_MLP_Fusion_Reward
...
for id in top_15_list:
    ...
    for ref_id in id_list:
        ...
        weights = f"regression_model_history/{game_name}_subject_{ref_id}_regression_model_{num_stacks}_stacks_history.pt"

        model = CNN_MLP_Fusion_Reward(num_frames=config.traj_length*num_stacks, use_activation=True)
    ...
    for model in model_list:
        ...
        with torch.inference_mode():
            simulated_reward = torch.zeros((image_tensor.shape[0])).to(device)

            for i in range(image_tensor.shape[0]):
                previous_reward = torch.zeros((1,1,num_stacks-1)).to(device)
                for j in range(1,num_stacks):
                    if i-j >= 0:
                        previous_reward[0,0,-j] = simulated_reward[i-j]
            
                simulated_reward[i] = model(image_tensor[i].unsqueeze(0),action_tensor[i].unsqueeze(0),previous_reward).squeeze().cpu()
    ...
    torch.save(reward, f"../Preference_guide_Data/{game_name}/{id}/{id}_simulated_reward_regression_history.pt")