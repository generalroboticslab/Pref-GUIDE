...
for i in range(len(mean_list)):
    if len(mean_list[i]) == 26:
        ...
        plt.plot(range(0,2*original_guide_weight-1,2),mean_list[i], marker='o', linewidth=12, color=color_dict[key_list[i]],linestyle="dashed")
    else:
        plt.plot(range(0,61,2),mean_list[i], marker='o', linewidth=12, color=color_dict[key_list[i]])
plt.legend(data_dict.keys(), loc='lower right',fontsize=45)
...
# plt.ylabel("Success Rate (%)", fontsize=36)
